    ...
    $(".destination-gallery, .sidebar-gallery").magnificPopup({
      ...
      image: {
        ...
        titleSrc: function (item) {
          return (
            item.el.attr("title") +
            ` <a href="https://www.indiatripplanners.com/" target="_blank" style="color: #f7921f;" rel="noopener noreferrer">&nbsp;Click Here</a>`
          );
        },
      },
    });
...
function initializeTopPlaceSlider() {
  if (topPlaceSliderElement && topPlaceNext && topPlacePrev) {
    ...
    // Optional: Pause autoplay on scroll
    let isScrolling;
    window.addEventListener('scroll', () => {
      clearTimeout(isScrolling);
      topPlaceSlider.autoplay.stop();
      isScrolling = setTimeout(() => topPlaceSlider.autoplay.start(), 1000);
    }, { passive: true });
  }
}
