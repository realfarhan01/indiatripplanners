        ...
        Try
            ...
            If FileUpload.HasFile Then
                ...
                If FileUpload.PostedFile.ContentType.Split("/")(0) = "image" Then
                    ...
                    CreateThumbnail(800, Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("productcategoryactualpath")) + File, Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("productcategorypath")) + File)
                End If
            End If
            Dim File2 As String = ""
            If FUBanner.HasFile Then
                If Not BLL.isImage(FUBanner.FileContent) Then
                    Exit Sub
                End If
                If FUBanner.PostedFile.ContentType.Split("/")(0) = "image" Then
                    'File = FileUpload.FileName
                    File2 = Guid.NewGuid().ToString & System.IO.Path.GetExtension(FUBanner.PostedFile.FileName)
                    FUBanner.SaveAs(Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("productcategoryactualpath")) + File2)
                    CreateThumbnail(1920, Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("productcategoryactualpath")) + File2, Server.MapPath(System.Configuration.ConfigurationManager.AppSettings("productcategorypath")) + File2)
                End If
            End If
            ...
            BLL.ExecNonQueryProc("AddUpdate_ProductCategory", "@ProductCategoryId", hfId.Value, "@UserName", user, "@ProductCategory", txtProductCategory.Text, "@ProductCategoryImage", File, "@BannerImage", File2, "@ShortDesc", txtShortDesc.Text, "@DeActivated", ddlStatus.SelectedValue, "@PageURL", PageURL, "@TourType", ddlTourType.SelectedValue)
            showdata()
            div1.Visible = False
            div2.Visible = True
    ...
    Protected Sub DataDisplay_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataDisplay.RowCommand
        ...
        ElseIf e.CommandName = "Deactivate1" Then
            ...
            If dr.Read Then
                ...
                ddlTourType.Text = dr("TourType").ToString()
            End If
            div1.Visible = True
            div2.Visible = False
